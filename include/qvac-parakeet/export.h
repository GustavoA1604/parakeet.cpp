...
// Symbol-visibility decorator for qvac-parakeet's public surface.
//
// Mirrors the LLAMA_API / WHISPER_API macros from sibling ggml-family
// libraries: when qvac-parakeet is built as a shared object, public
...
//
// Build flags:
//   - `QVAC_PARAKEET_SHARED` (PUBLIC define): set when building a
//     shared library. Consumers building against a shared
//     libqvac-parakeet must also define this so the symbols import
//     correctly on Windows.
//   - `QVAC_PARAKEET_BUILD`  (PRIVATE define): set inside the
...
#ifdef QVAC_PARAKEET_SHARED
#  if defined(_WIN32) && !defined(__MINGW32__)
#    ifdef QVAC_PARAKEET_BUILD
#      define QVAC_PARAKEET_API __declspec(dllexport)
#    else
#      define QVAC_PARAKEET_API __declspec(dllimport)
#    endif
#  else
#    define QVAC_PARAKEET_API __attribute__((visibility("default")))
#  endif
#else
#  define QVAC_PARAKEET_API
#endif
