...
int main(int argc, char ** argv) {
    ...
    qvac_parakeet::ctc::ParakeetCtcModel model;
    if (int rc = qvac_parakeet::ctc::load_from_gguf(gguf_path, model,
                                                    /*n_threads=*/0,
                                                    /*n_gpu_layers=*/0,
                                                    /*verbose=*/false); rc != 0) {
    ...
    if (model.model_type != qvac_parakeet::ctc::ParakeetModelType::CTC) {
        std::fprintf(stderr,
    ...
    std::vector<int32_t> ids = qvac_parakeet::ctc::ctc_greedy_decode(
        logits.data(), n_frames, model.vocab_size, model.blank_id);
    const std::string text = qvac_parakeet::ctc::detokenize(model.vocab, ids);
